...
def consistent(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if(bc.affects(var)):
			var2 = bc.otherVariable(var)
			if(assignment.isAssigned(var2)):
				if(not bc.isSatisfied(value,assignment.assignedValues[var2])):
					return False
	return True
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for i in range(len(values)):
		num += [countNumConstrained(assignment,csp,var,values[i])]
	values = [x for (y,x) in sorted(zip(num,values))]
	return values

# helper function to get the number for each var
def countNumConstrained(assignment,csp,var,value):
	num = 0
	for bc in csp.binaryConstraints:
		if(bc.affects(var)):
			var2 = bc.otherVariable(var)
			if(assignment.isAssigned(var2)):
				for value2 in assignment.varDomains[var2]:
					if(not bc.isSatisfied(value,value2)):
						num += 1
	return num
...
def forwardChecking(assignment, csp, var, value):
	...
	# update assignment domain according to inferences
	...
	for value in orderValuesMethod(assignment,csp,var):
		if(consistent(assignment,csp,var,value)):
			...
			if(inferences != None):				
				...
				if(result != None):
					return result
				# add back those removed inferences
				for inf in inferences:
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for value2 in domains[var2]:
		...
		for value in assignment.varDomains[var1]:
			if(constraint.isSatisfied(value,value2)):
				delval2 = False
				break
		if(delval2):
			inferences.add((var2, value2))
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if(bc.affects(var)):
			var2 = bc.otherVariable(var)
			if(not assignment.isAssigned(var2)):
				arcqueue.append((var,var2,bc))
	# braches of var's childs
	while(arcqueue):
		var1,var2,bc = arcqueue.popleft()
		inf = revise(assignment,csp,var1,var2,bc)
		if(inf == None):
			for infs in inferences:
				domains[infs[0]].add(infs[1])
			return None
		if(inf):
			for bc2 in csp.binaryConstraints:
				if(bc2.affects(var2)):
					var3 = bc2.otherVariable(var2)
					if(not assignment.isAssigned(var3)):
						arcqueue.append((var2,var3,bc2))		
		inferences |= inf
	return inferences
...
def AC3(assignment, csp):
	...
	"""Question 6"""
	"""YOUR CODE HERE"""
	# begins with all variables already in the queue
	domains = assignment.varDomains
	for bc in csp.binaryConstraints:
		for var in domains:
			if(bc.affects(var)):
